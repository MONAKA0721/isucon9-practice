  ...
  class Web < Sinatra::Base
    ...
    get '/users/transactions.json' do
      ...
      items = if item_id > 0 && created_at > 0
        # paging
        begin
          db.xquery("SELECT t1.*, t2.`account_name`, t2.`num_sell_items` FROM `items` AS t1 INNER JOIN `users` AS t2 ON t1.`seller_id` = t2.`id` WHERE (t1.`seller_id` = ? OR t1.`buyer_id` = ?) AND t1.`status` IN (?, ?, ?, ?, ?) AND (t1.`created_at` < ?  OR (t1.`created_at` <= ? AND t1.`id` < ?)) ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{TRANSACTIONS_PER_PAGE + 1}", user['id'], user['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT, ITEM_STATUS_CANCEL, ITEM_STATUS_STOP, Time.at(created_at), Time.at(created_at), item_id)
        rescue
          db.query('ROLLBACK')
        ...
        # 1st page
        begin
          db.xquery("SELECT t1.*, t2.`account_name`, t2.`num_sell_items` FROM `items` AS t1 INNER JOIN `users` AS t2 ON t1.`seller_id` = t2.`id` WHERE (t1.`seller_id` = ? OR t1.`buyer_id` = ?) AND t1.`status` IN (?, ?, ?, ?, ?) ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{TRANSACTIONS_PER_PAGE + 1}", user['id'], user['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT, ITEM_STATUS_CANCEL, ITEM_STATUS_STOP)
        rescue
          db.query('ROLLBACK')
          halt_with_error 500, 'db error'
        end
      ...
      item_details = items.map do |item|
        ...
        transaction_evidence = db.xquery('SELECT transaction_evidences.*, shippings.reserve_id AS shipping_reserved_id FROM `transaction_evidences` LEFT OUTER JOIN shippings ON transaction_evidences.id = shippings.transaction_evidence_id WHERE transaction_evidences.`item_id` = ?', item['id']).first
        unless transaction_evidence.nil?
          if transaction_evidence['shipping_reserved_id'].nil?
            db.query('ROLLBACK')
          ...
          ssr = begin
            if transaction_evidence['status'] == 'done'
              ...
              api_client.shipment_status(get_shipment_service_url, 'reserve_id' => transaction_evidence['shipping_reserved_id'])
            end
          rescue
            db.query('ROLLBACK')
    ...
    get '/users/:user_id.json' do
      ...
      items = if item_id > 0 && created_at > 0
        ...
        # db.xquery("SELECT t1.*, t2.`account_name`, t2.`num_sell_items` FROM `items` AS t1 INNER JOIN `users` ON t1.`seller_id` = t2.`id` WHERE t1.`seller_id` = ? AND t1.`status` IN (?, ?, ?) AND t1.`created_at` <= ? AND t1.`id` < ? ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", user_simple['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT, Time.at(created_at), item_id)
      else
        # 1st page
        ...
        # db.xquery("SELECT t1.*, t2.`account_name`, t2.`num_sell_items` FROM `items` AS t1 INNER JOIN `users` ON t1.`seller_id` = t2.`id` WHERE t1.`seller_id` = ? AND t1.`status` IN (?, ?, ?) ORDER BY t1.`created_at` DESC, t1.`id` DESC LIMIT #{ITEMS_PER_PAGE + 1}", user_simple['id'], ITEM_STATUS_ON_SALE, ITEM_STATUS_TRADING, ITEM_STATUS_SOLD_OUT)
      end
